...
interface OpportunityFormProps {
  initialData?: OpportunityFormState;
  onSubmit: (data: OpportunityFormState) => Promise<{ success: boolean; error?: string }>;
  submitLabel: string;
  loadingLabel: string;
  onSuccess?: () => void;
...
export default function OpportunityForm({
  initialData = EMPTY_OPPORTUNITY_FORM,
  onSubmit,
  submitLabel,
  loadingLabel,
  onSuccess,
  ...
  const [form, setForm] = useState<OpportunityFormState>(initialData);
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState(false);
  const [error, setError] = useState<string | null>(null);

  function setField(field: keyof OpportunityFormState) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setSuccess(true);
      if (initialData === EMPTY_OPPORTUNITY_FORM) {
